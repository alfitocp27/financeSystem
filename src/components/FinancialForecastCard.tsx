  ...
  return (
    <div className="bg-surface p-5 rounded-2xl sm:rounded-[14px] border border-border-default shadow-sm relative overflow-hidden">
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-border-default">
        ...
        {isDeficitExpected ? (
          <span className="inline-flex items-center gap-1 text-xs font-bold text-semantic-rose bg-semantic-rose-soft px-2.5 py-0.5 rounded-full border border-semantic-rose/20">
            <AlertCircle className="w-3.5 h-3.5" /> Risiko Defisit
          </span>
        ) : (
          <span className="inline-flex items-center gap-1 text-xs font-bold text-semantic-green bg-semantic-green-soft px-2.5 py-0.5 rounded-full border border-semantic-green/20">
      ...
      <div
        className={`p-3 rounded-xl text-xs flex items-start gap-2.5 ${
          isDeficitExpected
            ? 'bg-semantic-rose-soft border border-semantic-rose/20 text-semantic-rose-text'
            : 'bg-primary-soft border border-border-gold text-text-primary'
        }`}
      >
        <Lightbulb className={`w-4 h-4 shrink-0 mt-0.5 ${isDeficitExpected ? 'text-semantic-rose' : 'text-text-gold'}`} />
        <div className="leading-relaxed">
          {isDeficitExpected ? (
            <span>
              ...
              <strong className="text-text-primary">{formatCurrency(Math.max(0, dailyBurnRate - safeToSpend.dailySafeToSpend))}</strong> agar terhindar dari krisis akhir bulan.
            </span>
          ) : (
            <span>
              Kondisi finansialmu terkendali! Dengan laju pengeluaran saat ini, diproyeksikan tersisa{' '}
              <strong className="text-text-gold">{formatCurrency(projectedBalanceAtEnd)}</strong> saat siklus berakhir.
            </span>
          )}
        </div>
      </div>
